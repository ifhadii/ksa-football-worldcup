...
(function ($) {
    ...
    // :: 14.0 event ACTIVE CODE
    $('.event-area').each(function(index) {

        var count = index + 1;

        $(this).find('.event-items').removeClass('event-items').addClass('event-items-'+count);
        $(this).find('.event-item').removeClass('event-item').addClass('event-item-'+count);
        $(this).find('.event-btn').removeClass('event-btn').addClass('event-btn-'+count);
        
        var Shuffle = window.Shuffle;
        var Filter  = new Shuffle(document.querySelector('.event-items-'+count), {
            itemSelector: '.event-item-'+count,
            buffer: 1,
        })
    
        $('.event-btn-'+count).on('change', function (e) {
