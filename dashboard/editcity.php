...
if(isset($_POST['add_card'])) {
    ...
    $place_description = mysqli_real_escape_string($con, $_POST['place_description']);
    ...
    if(move_uploaded_file($tmp_name, "$uploads_dir/$new_file_name")) {
        mysqli_query($con, "INSERT INTO city_cards (city_id, place_name, place_image, place_description) 
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        $cards_result = mysqli_query($con, $cards_query);
                                        $card_count = 1;
                                        
                                        while($card = mysqli_fetch_array($cards_result)) {
                                            echo '<tr>
                                                <td>'.$card_count.'</td>
                                                <td>'.$card['place_name'].'</td>
                                                <td><img src="uploads/'.$card['place_image'].'" style="max-width: 100px;"></td>
                                                <td>'.substr($card['place_description'], 0, 100).'...</td>
                                                <td>
                                                    <a href="?id='.$todo.'&delete_card='.$card['id'].'" class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure?\')">Delete</a>
                                                </td>
                                            </tr>';
                                            $card_count++;
                                        }
